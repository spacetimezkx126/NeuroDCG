...
parser.add_argument('--news_path', type=str, default='./dataset/cn_score', help='Path to news data')
parser.add_argument('--price_path', type=str, default='./dataset/CMIN-CN/price/preprocessed', help='Path to price data')
...
class ProcessedDataset(Dataset):
    ...
    #         print(all_news[comp][self.all_date[date_ind]][0])
    ...
    #     return None
    def get_news_token(self, comp, date_ind):
        # print("217",date_ind,type(date_ind),self.all_date[date_ind])
        all_news = self.all_news_data
        if self.all_date[date_ind] in all_news[comp]:
            # print(all_news[comp][self.all_date[date_ind]][0])
            # print(all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0])
            # for b in all_news[comp][self.all_date[date_ind]][0]:
                # print(b,'original_text' in b)
    ...
    def _build_hetero_graphs_from_json(self):
        ...
        for comp in self.all_news_data:
            if not comp.startswith("."):
                ...
                # print(comp,self.all_news_data[comp].keys())
    ...
    for i in range(1):
        ...
        for epoch in range(epochs):
            ...
            for data in train_loader:
                optimizer.zero_grad()
                output, total_rank, ranks, out_cls, path, map_path, simu_all, grad_diff_all,_,_,_,_ = model(data.to(device),epoch=epoch)
