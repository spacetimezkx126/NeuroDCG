...
parser.add_argument('--news_path', type=str, default='./dataset/cmin_cn_extracted', help='Path to news data')
parser.add_argument('--price_path', type=str, default='./CMIN-CN/price/preprocessed', help='Path to price data')
...
class ProcessedDataset(Dataset):
    def __init__(self, data_path, split='train', transform=None, ds_name = 'CMIN-CN', news_path = './cmin_cn_extracted', price_path = './CMIN-CN/price/preprocessed', mode = 'load', data_num = 'all'):
        ...
        if self.mode == 'all':
            ...
            with gzip.open("hetero_data_cn_train.pkl.gz", "rb") as f:
                self.hetero_graphs_train = pickle.load(f)
            with gzip.open("hetero_data_cn_val.pkl.gz", "rb") as f:
                self.hetero_graphs_val = pickle.load(f)
            with gzip.open("hetero_data_cn_test.pkl.gz", "rb") as f:
    ...
    def load_sample_meta(self):
        ...
        hetero_data['score'].x = torch.randn((0,1))
    ...
    def _tokenize_and_pad_text(self, text, max_seq_len = 40):
        """
        Index the text and pad it to a fixed length
        """
    ...
    def _pad_texts_to_equal_length(self, tokenized_texts_org, max_docs = 8,max_seq_len = 40):
        """
        Pad or truncate the tokenized_texts so that the number reaches max_docs.
        Args:
            tokenized_texts (list of list): Indexed sequences of all texts in the current sample
            max_docs (int): The number of unified text sequences.
        Returns:
            padded_texts (list of list): The tokenized_texts after padding.
        """
        
        empty_text = [self.vocab["<pad>"]] * max_seq_len
    ...
    def get_news_token(self, comp, date_ind):
        ...
        if self.all_date[date_ind] in all_news[comp]:
            return [b['original_text'] if 'original_text' in b else (b['orginal_text'] if 'orginal_text' in b else "") for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
        return None
    
    def get_other_features(self, comp, date_ind):
        all_news = self.all_news_data
        if self.all_date[date_ind] in all_news[comp]:
            keywords = [b['features']['keywords'] if 'features' in b and 'keywords' in b['features'] else [] for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            sectors = [b['features']['sector_focus'] if 'features' in b and 'sector_focus' in b['features'] else [] for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            sentiment_score = [[b['features']['sentiment_score']] if 'features' in b and 'sentiment_score' in b['features'] else [0.0] for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            policy_related = [b['features']['policy_related'] if 'features' in b and 'policy_related' in b['features'] else "false" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            investement_strategy = [b['features']['investement_strategy'] if 'features' in b and 'investement_strategy' in b['features'] else "false" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            causal_factor = [b['features']['causal_factor'] if 'features' in b and 'causal_factor' in b['features'] else "false" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            causal_impact = [b['features']['causal_impact'] if 'features' in b and 'causal_impact' in b['features'] else "neutral" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            affected_by_time_series = [b['features']['affected_by_time_series'] if 'features' in b and 'affected_by_time_series' in b['features'] else "false" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            ts_effect_direction = [b['features']['ts_effect_direction'] if 'features' in b and 'ts_effect_direction' in b['features'] else "neutral" for b in all_news[comp][self.all_date[date_ind]][self.all_date[date_ind]][0]]
            return (keywords,sectors,sentiment_score,policy_related,investement_strategy,causal_factor,causal_impact,affected_by_time_series,ts_effect_direction)
        return None
    ...
    def trace_back_price(self, comp, date, k = 5):
        ...
        if price_window_st >= 0:
            if self.all_date_inverse[price_window[-1]] in self.all_price[comp]:
                return torch.cat([torch.tensor([self.all_price[comp][self.all_date_inverse[b]]]) for b in price_window if self.all_date_inverse[b] in self.all_price[comp]],dim=0)
            else:
                return None
        else:
            return None
    ...
    def cal_label(self, comp, date):
        ...
        if ind+1 == len(self.all_date) or self.all_date[ind+temp ] not in label_comp:
            ...
            return torch.tensor([1]) if float(label_comp[self.all_date[ind+ temp]]) > 0 else torch.tensor([0])
    ...
    def _data_from_json_to_heterodata(self, data_id, fields, comp):
        ...
        price = self.trace_back_price(comp, data_id)
        label = self.cal_label(comp, data_id)
        ...
        if news_by_date is not None:
            if news_by_date is not None:
                ...
                padded_text_pad = self._pad_texts_to_equal_length(padded_text)
            else:
                padded_text = [self._tokenize_and_pad_text("None")]
                padded_text_pad = self._pad_texts_to_equal_length([[]])
            ...
            counter = [0, 0, 0, 0, 0, 0]
            all_other_feat = [sentiment_score, [[6,6,6,6,6,6]]*len(sentiment_score), keywords, [[b] for b in news_by_date], [[0]]*len(sentiment_score), sectors]
            rela_all = [
            ...
            last_temp_score = 0
            last_temp_sector = 0
            list_fields = ['score','other','news','virtual','sector','keywords']
    ...
    def _build_hetero_graphs_from_json(self):
        ...
        for comp in self.all_news_data:
            ...
            if len(test_batches)!=0:
                self.hetero_graphs_test.append(Batch.from_data_list(test_batches))
        with gzip.open("hetero_data_cn_train.pkl.gz", "wb") as f:
            pickle.dump(self.hetero_graphs_train, f)
        with gzip.open("hetero_data_cn_val.pkl.gz", "wb") as f:
            pickle.dump(self.hetero_graphs_val, f)
        with gzip.open("hetero_data_cn_test.pkl.gz", "wb") as f:
...
if __name__ == '__main__':
    dataset = ProcessedDataset(data_path = "./multi-ts/causal-ts", news_path = './cmin_cn_extracted', price_path = './CMIN-CN/price/preprocessed')
    epochs = args.epochs
    phase = [10]
    import numpy as np
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            optimizer.zero_grad()
            output, total_rank, ranks, out_cls, path, map_path, simu_all, grad_diff_all = model(data.to(device),epoch=epoch)
            ...
            loss = loss1 + F.mse_loss(simu_all, grad_diff_all)
            loss.backward()
            i += 1
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                output, total_rank, ranks, out_cls, path, map_path, simu_all, grad_diff_all = model(data.to(device),epoch = 'e')
